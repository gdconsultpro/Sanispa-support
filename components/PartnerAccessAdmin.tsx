...
export function PartnerAccessAdmin({ partner }: { partner: PartnerAdminItem }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setAccesses(body.accesses); setLoadError("");
  ...
  async function save(mode: "create" | "link") {
    if (pending.current) return;
    pending.current = true; setSaving(true); setMessage("");
    try {
      const body = mode === "create" ? { mode, name, email, password } : { mode, name, email, existingUserId: existing, reactivate };
      ...
      if (result.conflict && result.existingUserId) { setExisting(result.existingUserId); setPassword(""); setMessage(result.message); return; }
  ...
  return <section className="mt-6 border-t border-sanispa-line pt-5">
    ...
    {loadError ? <p role="alert" className="mt-3 text-sm text-red-800">{loadError} <button onClick={() => void load()} className="underline">Réessayer</button></p> : accesses.length ? <ul className="my-3 grid gap-2">{accesses.map(access => <li key={access.user_id} className="rounded-md bg-sanispa-ice p-3 text-sm">
    ...
    <form onSubmit={event => { event.preventDefault(); void save(existing ? "link" : "create"); }} className="mt-4 grid gap-4 sm:grid-cols-2" aria-busy={saving}>
      ...
      {!existing ? <><Field label="Mot de passe provisoire (12 caractères minimum)" name="partner-temporary-password" type={show ? "text" : "password"} value={password} onChange={setPassword} required minLength={12} maxLength={128} autoComplete="new-password" />
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={show} onChange={e => setShow(e.target.checked)} />Afficher le mot de passe provisoire</label></> : <div className="rounded-md bg-amber-50 p-3 text-sm sm:col-span-2">
