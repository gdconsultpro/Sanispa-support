      ...
      <form onSubmit={submit} className="space-y-4 rounded-md border border-sanispa-line bg-white p-4 shadow-soft sm:p-6">
        {questions.map((question) => {
            ...
            if (question.type === "radio") {
                return (<fieldset key={question.id} className="rounded-md border border-sanispa-line p-4">
                ...
                <div className="mt-3 grid gap-2">
                  {question.options?.map((option) => (<label key={option} className="flex min-h-11 items-center gap-3 rounded-md bg-sanispa-ice px-3 py-2 text-sm font-semibold text-sanispa-navy">
                      <input type="radio" name={question.id} checked={draft.answers[question.id] === option} onChange={() => updateAnswer(question.id, option)} required={question.required}/>
                      {option}
                    </label>))}
                </div>
              </fieldset>);
            ...
            if (question.type === "textarea") {
                return (<TextAreaField key={question.id} label={question.label} name={question.id} value={draft.answers[question.id] ?? ""} onChange={(value) => updateAnswer(question.id, value)} required={question.required}/>);
            }
            return (<Field key={question.id} label={question.label} name={question.id} value={draft.answers[question.id] ?? ""} onChange={(value) => updateAnswer(question.id, value)} required={question.required} type={question.type}/>);
        })}
