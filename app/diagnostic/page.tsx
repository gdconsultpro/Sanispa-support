...
export default function DiagnosticPage() {
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    writeDraft({ ...draft, answers: draft.answers ?? {}, photos: draft.photos ?? {} });
    ...
    <AppShell compact>
      ...
      <form onSubmit={submit} className="space-y-5 rounded-md border border-sanispa-line bg-white p-4 shadow-soft sm:p-6">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Adresse" name="address" value={draft.address} onChange={(value) => update("address", value)} required />
          <Field label="Marque du spa" name="spaBrand" value={draft.spaBrand} onChange={(value) => update("spaBrand", value)} required />
          ...
          <SelectField
            ...
            onChange={(value) => update("powerSupply", value as DiagnosticDraft["powerSupply"])}
            required
            options={[
