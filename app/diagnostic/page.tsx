...
export default function DiagnosticPage() {
    return (<Suspense fallback={<AppShell compact><div className="rounded-md border border-sanispa-line bg-white p-5 text-sanispa-steel">Chargement du formulaire...</div></AppShell>}>
      <DiagnosticContent />
    </Suspense>);
}
function DiagnosticContent() {
    ...
    async function submit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const next = { ...draft, draftId: draft.draftId || crypto.randomUUID(), spaId: selectedSpaId || undefined };
            writeDraft(next);
            ...
            await saveDraft(next, "/questionnaire");
    ...
    return (<AppShell compact>
      ...
      <form onSubmit={submit} className="space-y-5 rounded-md border border-sanispa-line bg-white p-4 shadow-soft sm:p-6">
        {spas.length > 0 ? (<SelectField label="Choisir le spa concerné" name="selectedSpa" value={selectedSpaId} onChange={selectSpa} options={spas.map((spa) => ({
                value: spa.id,
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <SelectField label="Type d'installation" name="installationType" value={draft.installationType} onChange={(value) => update("installationType", value as DiagnosticDraft["installationType"])} required options={[
            { value: "interieur", label: "Intérieur" },
            { value: "exterieur", label: "Extérieur" }
        ]}/>
          <SelectField label="Type de problème" name="problemType" value={draft.problemType} onChange={(value) => update("problemType", value as DiagnosticDraft["problemType"])} required options={problemTypes}/>
        </div>
