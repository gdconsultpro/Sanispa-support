...
2. Pour une base neuve seulement, exécuter `supabase/schema.sql`. Sur la base existante correspondant au dépôt de juin, exécuter uniquement les quatre nouveaux fichiers de `supabase/migrations/`, dans l’ordre alphabétique : `01_client_access`, `02_submission`, `03_payments`, `04_sav`.
...
4. Vérifier les variables Vercel : `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, le prix Stripe eau indiqué dans `lib/questions.ts`, `OPENAI_API_KEY`, `RESEND_API_KEY`, `ADMIN_NOTIFICATION_EMAIL`, `EMAIL_FROM`. Les secrets serveur ne doivent jamais porter le préfixe `NEXT_PUBLIC_`.
...
- L’administration conserve son mode de connexion Basic Auth, renforcé par un contrôle dans chaque route. Des comptes collaborateurs individuels avec MFA constituent une évolution ultérieure.
